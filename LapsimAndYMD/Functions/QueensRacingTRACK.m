function [tr] = QueensRacingTRACK(filename, mode, log_mode)
%% Code to generate a track model for the Queen's Racing LapSim
%
...
if strcmp(mode,'logged data')
    ...
    table_el = readData(filename,'Elevation') ;
    table_bk = readData(filename,'Banking') ;
    table_gf = readData(filename,'Grip Factors') ;
    table_sc = readData(filename,'Sectors') ;
    
end

%% Track model name

[folder_status,folder_msg] = mkdir('QueensRacingTRACK Tracks') ;
trackname = "QueensRacingTRACK Tracks/"+info.name+"_"+info.config+"_"+info.direction ;
...
figtitle = ["QueensRacingTRACK","Track Name: "+info.name,"Configuration: "+info.config,"Mirror: "+info.mirror,"Date & Time: "+datestr(now,'yyyy/mm/dd')+" "+datestr(now,'HH:MM:SS')] ;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [data] = readData(workbookFile,sheetName,startRow,endRow)
    % Input handling
    % If no sheet is specified, read first sheet
    if nargin == 1 || isempty(sheetName)
        sheetName = 1;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [header,data] = readLoggedData(filename,headerStartRow,headerEndRow,dataStartRow,dataEndRow)
    ...
    headerArray = textscan(fileID, header_formatSpec, headerEndRow(1)-headerStartRow(1)+1, 'Delimiter', delimiter, 'TextType', 'string', 'HeaderLines', headerStartRow(1)-1, 'ReturnOnError', false, 'EndOfLine', '\r\n');
    for block=2:length(headerStartRow)
        frewind(fileID);
        dataArrayBlock = textscan(fileID, header_formatSpec, headerEndRow(block)-headerStartRow(block)+1, 'Delimiter', delimiter, 'TextType', 'string', 'HeaderLines', headerStartRow(block)-1, 'ReturnOnError', false, 'EndOfLine', '\r\n');
    ...
    % Read columns of data according to the format.
    dataArray = textscan(fileID, data_formatSpec, dataEndRow(1)-dataStartRow(1)+1, 'Delimiter', delimiter, 'TextType', 'string', 'HeaderLines', dataStartRow(1)-1, 'ReturnOnError', false, 'EndOfLine', '\r\n');
    for block=2:length(dataStartRow)
        frewind(fileID);
        dataArrayBlock = textscan(fileID, data_formatSpec, dataEndRow(block)-dataStartRow(block)+1, 'Delimiter', delimiter, 'TextType', 'string', 'HeaderLines', dataStartRow(block)-1, 'ReturnOnError', false, 'EndOfLine', '\r\n');
